...
export function Sidebar({ guest, room, onlineUsers, open, onClose, onGlobal, onCreate, onChangeAlias }: Props) {
  return <>
    ...
    <aside className={`fixed inset-y-0 left-0 z-40 flex w-72 flex-col border-r border-slate-800 bg-slate-900 transition-transform md:static md:translate-x-0 ${open ? 'translate-x-0' : '-translate-x-full'}`}>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6">
        ...
        <button onClick={onGlobal} className={`mt-3 flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm font-medium transition ${room?.id === 'global' ? 'bg-indigo-500/15 text-indigo-200 ring-1 ring-indigo-500/20' : 'text-slate-300 hover:bg-slate-800'}`}><Hash size={18} className="text-indigo-400" /> Global chat <Radio size={13} className="ml-auto text-emerald-400" /></button>
        {room && room.id !== 'global' && <div className="mt-2 flex items-center gap-3 rounded-xl bg-indigo-500/15 px-3 py-3 text-sm font-medium text-indigo-200 ring-1 ring-indigo-500/20"><Hash size={18} className="text-indigo-400" /><span className="truncate">{room.name}</span></div>}
