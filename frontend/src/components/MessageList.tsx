...
interface MessageRowProps { message: ChatMessage; own: boolean }
...
const MessageRow = memo(function MessageRow({ message, own }: MessageRowProps) {
  return <article className={`flex gap-3 ${own ? 'justify-end' : 'justify-start'}`}>
    ...
    <div className={`max-w-[85%] sm:max-w-[70%] ${own ? 'items-end' : 'items-start'} flex flex-col`}>
      <div className={`mb-1 flex items-baseline gap-2 text-xs ${own ? 'flex-row-reverse' : ''}`}>
        ...
        <time className="text-slate-500" dateTime={message.createdAt}>{timeFormatter.format(new Date(message.createdAt))}</time>
      </div>
      <div className={`rounded-2xl px-4 py-3 text-sm leading-6 shadow-sm ${own ? 'rounded-tr-sm bg-indigo-500 text-white' : 'rounded-tl-sm border border-slate-700 bg-slate-800 text-slate-100'}`}>
...
export const MessageList = memo(function MessageList({ messages, clientId, roomName }: Props) {
  ...
  return <div className="flex-1 overflow-y-auto px-4 py-6 sm:px-8" aria-live="polite">
    {messages.length === 0 ? <div className="mx-auto flex h-full max-w-sm flex-col items-center justify-center text-center">
      ...
      {messages.map((message) => <MessageRow key={message.id} message={message} own={message.clientId === clientId} />)}
      <div ref={endRef} />
    </div>}
  </div>;
});
