...
import { roomIdFromPath, roomPath } from './lib/rooms';
...
export default function App() {
  ...
  return <div className="flex h-dvh min-h-[480px] overflow-hidden bg-slate-950 text-white">
    <Sidebar guest={guest} room={chat.room} onlineUsers={chat.onlineUsers} open={sidebarOpen} onClose={() => setSidebarOpen(false)} onGlobal={() => navigate('global')} onCreate={() => { setSidebarOpen(false); setCreateOpen(true); }} onChangeAlias={() => { setGuest(null); setSidebarOpen(false); }} />
    ...
    {createOpen && <CreateRoomDialog onClose={() => setCreateOpen(false)} onCreated={(room: Room) => { setCreateOpen(false); navigate(room.id); }} />}
  </div>;
}
